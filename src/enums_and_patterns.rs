...
/*
*   The 'if let' syntax lets you combine 'if' and 'let' into a less verbose way to handle values that match one pattern while ingnoring
*   the rest. Consider the pogram in the code below that matches on an 'Option<u8>' value in the 'config_max' variable but only wants
*   to execute code if the value is the 'Some' variant.
*/
...
*   We don't want to do anything with the 'None' value. To satisfy the 'match' expression, we have to add '_ => ()' after processing
*   just one variant, which is annoying boilerplater code to add.
*   
...
/*
*   The syntax 'if let' takes a pattern and an expression sperarated by an equal sign. It works the same way as a 'match', where the
*   expression is given to the 'match' and the pattern is its first arm. In this case, the pattern is 'Some(max)', and the 'max' binds
*   to the value inside the 'Some'. We can then use 'max' in the body of the 'if let' block in the same way we use 'max' in the corresponding
*   'match' arm. The code in the 'if let' block isn't run if the value doesn't match the pattern.
*   
*   Using 'if let' means less typing, less indentation, and less boilerplate code. However, you lose the exhaustive checking that 'match'
*   enforces. Choosing between 'match' and 'if let' depends on what you're doing in your particular situation and whether gaining conciseness
*   is an appropriate trade-off for losing exhaustive checking.
*   
...
*   
*   We can include an 'else' with an 'if let'. The block of code that goes with the 'else' is the same as the block of code that would go
*   with the '_' case in the 'match' expression that is equivalent to the 'if let' and 'else'. Recall that 'Coin' enum definition from before,
*   where the 'Quarter' variant also held a 'UsState' value. If we wanted to count all non-quarter coins we se while also announcing the state of
...
/*
*   If you have a situation in which your program has logic that is too verbose to express using a 'match', remember that 'if let' is in your
